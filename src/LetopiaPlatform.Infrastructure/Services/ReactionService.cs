...
public class ReactionService : IReactionService
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (targetType.Equals("Post", StringComparison.OrdinalIgnoreCase))
        {
            var post = await _postRepo.GetByIdAsync(targetId);
            if (post == null || post.IsDeleted)
                throw new KeyNotFoundException("Post not found.");

            return await ToggleReactionForTarget(postId: post.Id, commentId: null, targetType: "Post", request.ReactionType, userId, ct);
        }
        else if (targetType.Equals("Comment", StringComparison.OrdinalIgnoreCase))
        {
            var comment = await _commentRepo.GetByIdAsync(targetId);
            if (comment == null || comment.IsDeleted)
                throw new KeyNotFoundException("Comment not found.");

            return await ToggleReactionForTarget(postId: comment.PostId, commentId: comment.Id, targetType: "Comment", request.ReactionType, userId, ct);
        }
        else
        {
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            var downvotes = reactions.Count(r => r.ReactionType == ReactionType.Downvote);

            // Update entity counters
            if (targetType == "Post")
            {
                var post = await _postRepo.GetByIdAsync(postId);
                if (post != null) post.Upvotes = upvotes - downvotes;
                await _unitOfWork.SaveChangesAsync(ct);
            }
            else if (targetType == "Comment" && commentId.HasValue)
            {
                var comment = await _commentRepo.GetByIdAsync(commentId.Value);
                if (comment != null) comment.Upvotes = upvotes - downvotes;
                await _unitOfWork.SaveChangesAsync(ct);
            }
